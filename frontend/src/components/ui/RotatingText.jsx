...
const RotatingText = forwardRef((props, ref) => {
  ...
  const elements = useMemo(() => {
    ...
    if (splitBy === 'characters') return currentText.split(' ').map((word, i) => ({ characters: splitIntoCharacters(word), needsSpace: i !== currentText.split(' ').length - 1 }))
  ...
  }, [staggerFrom, staggerDuration, reduced])
  ...
  useEffect(() => { if (!auto || reduced) return; const id = setInterval(next, rotationInterval); return () => clearInterval(id) }, [next, rotationInterval, auto, reduced])

  if (reduced) return <span className={mainClassName}>{texts[currentTextIndex]}</span>

  return (
    <motion.span className={`inline-flex flex-wrap whitespace-pre-wrap relative ${mainClassName || ''}`} {...rest} layout transition={transition}>
      <span className="sr-only">{texts[currentTextIndex]}</span>
      <AnimatePresence mode={animatePresenceMode} initial={animatePresenceInitial}>
        <motion.span key={currentTextIndex} className={`${splitBy === 'lines' ? 'flex flex-col w-full' : 'flex flex-wrap whitespace-pre-wrap relative'} ${splitLevelClassName || ''}`} layout aria-hidden="true">
              ...
              <span key={wordIndex} className={`inline-flex ${splitLevelClassName || ''}`}>
                {wordObj.characters.map((char, charIndex) => (
                  <motion.span key={charIndex} initial={initial} animate={animate} exit={exit}
                    ...
                    className={`inline-block ${elementLevelClassName || ''}`}>{char}</motion.span>
                ))}
