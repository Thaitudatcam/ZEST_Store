...
const ShinyText = ({ text, disabled = false, speed = 2, className = '', color = '#b5b5b5', shineColor = '#ffffff', spread = 120, pauseOnHover = false, direction = 'left', delay = 0 }) => {
  ...
  useAnimationFrame(time => {
    if (disabled || isPaused || reduced) { lastTimeRef.current = null; return }
  ...
  if (reduced) return <span className={className} style={{ color }}>{text}</span>

  return (
    <motion.span className={`inline-block ${className}`} style={{ ...gradientStyle, backgroundPosition }}
      onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>{text}</motion.span>
  )
}
