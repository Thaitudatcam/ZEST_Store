  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen(!open)}
        className="relative p-2 rounded-xl hover:bg-gold/10 transition-colors text-stone hover:text-gold">
        <Bell className="h-5 w-5" />
        {unreadCount > 0 && (
          <span className="absolute -top-0.5 -right-0.5 bg-bordeaux text-noir text-[10px] font-bold min-w-[18px] h-[18px] flex items-center justify-center rounded-full px-1">
            ...
            {notifications.length === 0 ? (
              ...
              notifications.map((n) => (
                <button key={n.maThongBao} onClick={() => handleClick(n)}
                  className={`w-full text-left px-4 py-3 border-b border-gold/5 hover:bg-gold/5 transition-colors flex items-start gap-3 ${!n.daDoc ? 'bg-gold/5' : ''}`}>
                  <div className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${!n.daDoc ? 'bg-gold' : 'bg-transparent'}`} />
