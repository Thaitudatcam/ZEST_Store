...
export default function ProductCard({ variant, mode = 'grid', onAdd, onQtyChange, cartQty = 0 }) {
  ...
  if (mode === 'modal') {
    return (
      <div className={`flex items-start gap-3 bg-white rounded-xl border p-3 transition-all hover:shadow-md ${cartQty > 0 ? 'border-[var(--primary-color)] ring-1 ring-[var(--primary-color)]/20' : 'border-stone/10'}`}>
  ...
  return (
    <div className="bg-white rounded-xl border border-stone/10 overflow-hidden hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5 group">
