...
export default function Navbar() {
  ...
  return (
    <nav className="bg-ivory/90 backdrop-blur-md border-b border-gold/15 sticky top-0 z-50">
        ...
        <div className="flex items-center justify-between h-16 gap-4">
            ...
            <form onSubmit={handleSearchSubmit} className="w-full">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-10 py-2 border border-noir-600/15 rounded-full text-sm bg-ivory/70 text-ink placeholder-stone focus:outline-none focus:ring-2 focus:ring-gold/40 focus:border-gold focus:bg-ivory transition"
                />
                {searchLoading && <Loader className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gold animate-spin" />}
              </div>
            </form>
            {showSuggestions && suggestions.length > 0 && (
              <div className="absolute top-full mt-1 left-0 right-0 bg-ivory border border-gold/20 rounded-xl shadow-lux z-50 py-2 max-h-80 overflow-y-auto">
                {suggestions.map((p) => (
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            {user ? (
              ...
              <Link to="/wishlist" className="relative text-ink-soft hover:text-gold-dark transition">
                ...
                {wishlistCount > 0 && (
                  <span className="absolute -top-2 -right-2 bg-bordeaux text-noir text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1 leading-none animate-scale-in">
              ...
              <Link to="/vouchers" className="relative text-ink-soft hover:text-gold-dark transition">
                ...
                <div className="relative" ref={dropdownRef}>
                  ...
                  {dropdownOpen && (
                    <div className="absolute right-0 mt-2 w-52 bg-ivory rounded-xl shadow-lux border border-gold/15 py-2 z-50">
          ...
          <div className="flex md:hidden items-center gap-2">
            ...
            <Link to="/cart" className="relative text-ink-soft">
              <ShoppingCart className="h-5 w-5" />
              {count > 0 && <span className="absolute -top-2 -right-2 bg-bordeaux text-noir text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1">{count}</span>}
            </Link>
      ...
      {searchOpen && (
        <div className="md:hidden border-t border-gold/15 px-4 py-3 bg-ivory">
          ...
          {showSuggestions && suggestions.length > 0 && (
            <div className="mt-2 border border-gold/15 rounded-xl bg-ivory shadow-lux max-h-60 overflow-y-auto">
              {suggestions.map((p) => (
                <Link key={p.maSanPham} to={`/products/${p.slug}`}
      ...
      {open && (
        <div className="md:hidden border-t border-gold/15 px-4 py-4 space-y-1 bg-ivory">
