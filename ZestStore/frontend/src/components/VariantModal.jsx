...
export default function VariantModal({ variants, images, onConfirm, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 animate-fade-in" onClick={onClose}>
      <div className="bg-ivory rounded-2xl max-w-lg w-full mx-4 max-h-[80vh] overflow-y-auto animate-scale-in" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b">
          <h3 className="font-bold text-lg">Chọn phân loại</h3>
          <button onClick={onClose} className="text-stone hover:text-stone"><X className="h-5 w-5" /></button>
        </div>
        ...
        <div className="p-4">
            ...
            {variants.map(v => {
              ...
              return (
                <button key={v.maBienThe} onClick={() => { if (!disabled) { setSel(v.maBienThe); setQty(1) } }} disabled={disabled}
                  className={`text-left border rounded-xl p-3 transition ${active ? 'border-gold ring-2 ring-blue-200' : disabled ? 'bg-ivory-100 border-stone/20 cursor-not-allowed' : 'hover:border-stone/30'}`}>
                  <div className="aspect-square bg-ivory-100 rounded-lg overflow-hidden mb-2 relative">
                  ...
                  <p className="text-gold font-bold text-sm">{VND(v.gia || 0)}</p>
                  <p className={`text-xs ${disabled ? 'text-bordeaux font-semibold' : 'text-stone'}`}>{disabled ? 'Hết hàng' : `Kho: ${stock}`}</p>
                </button>
              )
            })}
          </div>
          ...
          <div className="flex items-center gap-3 mt-4">
            ...
            <div className="flex border rounded-lg">
              <button onClick={() => setQty(Math.max(1, qty - 1))} className="px-3 py-1.5 hover:bg-ivory-100">-</button>
              <span className="px-4 py-1.5 border-x min-w-[2.5rem] text-center text-sm">{qty}</span>
              <button onClick={() => setQty(Math.min(selectedStock, qty + 1))} className="px-3 py-1.5 hover:bg-ivory-100" disabled={qty >= selectedStock}>+</button>
            </div>
          </div>
        </div>
        ...
        <div className="border-t p-4">
          <button onClick={() => onConfirm(selected?.maBienThe, qty)} disabled={isOutOfStock}
            className={`w-full font-semibold py-3 rounded-xl transition flex items-center justify-center gap-2 ${isOutOfStock ? 'bg-ivory-100 text-stone cursor-not-allowed' : 'bg-gold text-noir hover:bg-gold-hover'}`}>
