        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {items.map((i) => (
            <div key={i.maSanPham} className="relative">
              <button onClick={() => { removeWishlist(i.maSanPham); setItems(items.filter((x) => x.maSanPham !== i.maSanPham)) }} className="absolute top-2 right-2 bg-ivory rounded-full p-1.5 shadow z-10 text-bordeaux hover:text-bordeaux"><Heart className="h-4 w-4 fill-red-400" /></button>
