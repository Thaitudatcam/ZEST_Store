      ...
      <div className="bg-ivory rounded-xl shadow-sm border p-8">
        ...
        {step === 1 ? (
          ...
          <form onSubmit={handleReset} className="space-y-4">
            <p className="text-sm text-stone text-center">Mã OTP đã được gửi đến <strong>{email}</strong></p>
            <input type="text" value={otp} onChange={(e) => setOtp(e.target.value)}
              placeholder="Mã xác thực (6 số)" required maxLength={6}
              className="w-full border rounded-lg px-4 py-2 text-center text-lg tracking-widest focus:outline-none focus:ring-2 focus:ring-gold" />
            <div className="relative">
