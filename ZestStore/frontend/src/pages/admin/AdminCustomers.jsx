    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="relative">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Tìm khách hàng..." className="pl-9 pr-4 py-2 border rounded-lg text-sm w-64 focus:outline-none focus:ring-2 focus:ring-gold" />
        </div>
      </div>
      ...
      <div className="bg-ivory rounded-2xl shadow-sm border overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {paged.map((c) => (
                <tr key={c.maNguoiDung} className="hover:bg-ivory-100">
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className="flex justify-center gap-1">
                      <button onClick={() => { setDetail(c); getCustomerDiem(c.maNguoiDung).then(setDetailDiem).catch(() => setDetailDiem(null)) }} className="p-1.5 text-gold hover:bg-gold/10 rounded-lg"><Eye className="h-4 w-4" /></button>
        ...
        {paged.length === 0 && <p className="text-center text-stone py-8">Chưa có khách hàng</p>}
            ...
            {detailDiem && (
              <div className="mt-4 p-3 bg-gold/10 border border-gold/20 rounded-lg">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="font-medium text-gold-hover">Điểm tích lũy:</span>
                  <span className="text-lg font-bold text-gold-hover">{detailDiem.soDiem?.toLocaleString()}</span>
                </div>
                <div className="flex gap-4 mt-1 text-xs text-gold">
                  <span>Đã tích: {detailDiem.tongTichLuy?.toLocaleString()}</span>
                  <span>Đã dùng: {detailDiem.tongSuDung?.toLocaleString()}</span>
                </div>
              </div>
            )}
            <button onClick={() => setDetail(null)} className="mt-3 w-full border rounded-lg py-2 text-sm font-semibold hover:bg-ivory-100">Đóng</button>
          </div>
        </div>
      )}
