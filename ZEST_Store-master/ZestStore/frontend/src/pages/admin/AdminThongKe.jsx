...
import { getRevenueByDay, getRevenueByMonth, getRevenueByYear, getBestSelling, getOrderStats, getStats } from '../../api/admin'
...
export default function AdminThongKe() {
  ...
  const loadRevenue = async () => {
    try {
      let data
      if (tab === 'day') data = await getRevenueByDay(tuNgay, denNgay)
      else if (tab === 'month') data = await getRevenueByMonth(nam)
      else data = await getRevenueByYear()
      setRevenueData(Array.isArray(data) ? data : [])
    } catch { setRevenueData([]) }
  }

  useEffect(() => { load() }, [])
  useEffect(() => { loadRevenue() }, [tab, tuNgay, denNgay, nam])
      ...
      <div className="bg-white rounded-2xl shadow-sm border p-6">
        ...
        {tab === 'month' && (
          <div className="mb-4">
            ...
            <input type="number" value={nam} onChange={e => setNam(Number(e.target.value))} className="border rounded-lg px-3 py-1.5 text-sm ml-2 w-24" />
          </div>
        )}
        {revenueData.length > 0 ? (
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={revenueData}>
              ...
              <XAxis dataKey={tab === 'day' ? 'ngay' : tab === 'month' ? 'thang' : 'nam'} tick={{ fontSize: 12 }} stroke="#94a3b8" />
